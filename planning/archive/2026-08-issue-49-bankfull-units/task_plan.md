...
- `precip` default becomes **`NULL`** meaning "omit the term". A literal `1` would otherwise be read
  as 1 mm = 0.1 cm/yr and silently scale depth by 0.6089 — shallower than dropping the term.
...
- [x] Convert inside the function: `area_km2 <- contrib / 100`, `precip_cm <- precip / 10`.
- [x] `precip = NULL` default → multiplier exactly `1`; scalar or `SpatRaster` interpreted as mm.
      Keep the existing negative-value clamp on both inputs.
- [x] Roxygen: state **hectares** on `streams` and **mm** on `precip`; delete the current
...
- [x] Propagate the `precip = NULL` default and units wording to `R/fl_flood_model.R` and
      `R/fl_valley_confine.R` (both currently default `precip = 1`).
- [x] `devtools::document()`; read its output — no unexpected `.Rd` written, `git diff NAMESPACE`
      empty.
- [x] Phase 1 tests pass; full `devtools::test()` green.

## Phase 3: Measure, then re-anchor the scenarios

- [x] Re-run `ff02/ff04/ff06` on the bundled 10 m tile; record corrected cell counts and hectares
      against the as-coded 320.8 / 476.8 / 536.4 ha.
- [x] Re-run on `inst/vignette-data/pars_dem.tif` + `pars.gpkg` (30 m MRDEM) for the resolution
      comparison. Confirm the corrected result is a strict subset (0 cells gained).
- [x] Values stay 2 / 4 / 6. Update `inst/extdata/flood_scenarios.csv` `ecological_process` text and
      the `flood_factor` row of `inst/extdata/flood_params.csv` to state input units and the
      resolution equivalence.
- [x] Add the guidance that at 30 m MRDEM the functional-floodplain equivalent is nearer
...
- [x] Update `fl_scenarios()` roxygen ladder to match.

## Phase 4: Documentation

- [x] `inst/notes/methodology.md` — the two-model formula table states **no units at all** for
      either row; add them. Update the scenario table, add resolution guidance, refresh the stale
      live cell counts (53,635 / 521,028).
- [x] `inst/notes/floodplain_interpretation.md` — rewrite section 4 (defect fixed, not pending),
...
- [x] `inst/research/vca_parameter_rationale.md` — already correct; note that the code now agrees.
- [x] `README.md:45` — "underestimates flood depth by ~4x" is the as-coded ratio (3.87x); corrected
      is **2.35x**.
- [x] `vignettes/valley-confinement.Rmd` — Step 2 prose ("~2 m vs ~8 m flood depth") and the formula
      block. Vignette area figures compute live and need no edit.
- [x] `vignettes/pars-floodplain.Rmd` — check the `ff04` framing against the new guidance.
- [x] Fix the `@nagel_etal2014LandscaleScale` typo in `flood_params.csv` line 5 while there.

## Phase 5: Regenerate the shipped vignette artifacts

- [x] Regenerate `inst/vignette-data/pars_valleys.tif` and the `floodplain` layer of `pars.gpkg`
...
- [x] Record before/after cell counts and hectares; confirm strict-subset.
- [x] `pars_meta.rds` carries no ff value — verify before assuming it needs no change.

## Phase 6: Release and downstream notice

- [x] `NEWS.md` — prominent **result-changing** entry with measured before/after on both shipped
...
- [x] Phase 1 tests fail on unmodified code, pass after Phase 2 — recorded, not asserted from memory
- [x] `devtools::test()` green; `lintr::lint_package()` no new lints vs `HEAD` baseline
- [x] `devtools::document()` output read; `NAMESPACE` diff empty
- [x] `devtools::check()` clean before the version bump
- [x] Corrected delineation is a strict subset of the as-coded one on **both** shipped datasets
