...
- [x] `R/fl_valley_confine.R` - signature becomes
...
- [x] Roxygen: `@param area_field` states **upstream contributing area in hectares** and
...
- [x] `R/fl_stream_rasterize.R` - roxygen note that its output is *not* interchangeable
...
- [x] `tests/testthat/test-fl_valley_confine.R` - 20 calls; note `:91` rasterizes on
      `"channel_width"` and `:91-101` asserts the raster path equals the `sf` path, so
      both sides move to `"upstream_area_ha"` in lockstep
- [x] `tests/testthat/test-fl_valley_attribute.R:10, :186`
- [x] Roxygen examples: `R/fl_valley_confine.R:89-106`, `R/fl_valley_attribute.R:101`
- [x] `README.md:24`
- [x] Vignettes: `valley-confinement.Rmd` (4), `stac-dem.Rmd.orig` **and** the baked
      `stac-dem.Rmd` (4 each), `pars-floodplain.Rmd:232`
- [x] `data-raw/wsg_vignette_data.R:258`
- [x] `CLAUDE.md:273` - the two-argument pipeline snippet, which this change makes an error

## Phase 4: Docs, notes, release bookkeeping

- [x] `devtools::document()` - regenerate `man/fl_valley_confine.Rd`,
...
- [x] `NEWS.md` - new `# flooded 0.6.0` section: the rename, why the default was wrong,
...
- [x] `soul/conventions/cartography.md:120` - `flooded::fl_valley_confine(dem, streams)`
      ...
      branch + PR.
- [x] PR body notes `floodplains/scripts/floodplain_lcc/02_floodplain_model.R:148` as a
...
- [x] `Rscript -e 'lintr::lint_package()'` - no new lints vs `HEAD` baseline
- [x] `Rscript -e 'devtools::check()'` - examples execute, vignettes build
- [x] Measurement re-run on the bundled tile at package defaults: record cell counts and
...
- [x] Restore-the-bug check on the new required-argument test: re-add a
...
- [x] `/code-check` clean on each commit; PWF checkboxes match landed work;
