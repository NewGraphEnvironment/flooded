...
#' | 30 m MRDEM | up to 7 |
#'
#' So `ff04` on 25 m TRIM and `ff03` on 10 m aim at the same ecological thing,
#' and `ff04` on 30 m MRDEM is a *conservative* functional floodplain rather
#' than a centred one. Comparing `ff04` across resolutions compares two
#' different things.
#'
#' These values are unchanged across the 0.5.0 units fix (flooded#49) because
#' they were taken from the literature ladder to begin with. What changed is
#' that they now behave as labelled — before the fix, `ff02` / `ff04` / `ff06`
#' were really 7.19 / 14.37 / 21.56 times bankfull depth.
#'
