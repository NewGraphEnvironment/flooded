...
- **`channel_buffer`** — auto-detected when streams have a `channel_width`
  ...
  the output. This is a DEM correction, not a habitat buffer.
...
```{r plot-features, fig.width = 7, fig.height = 8, fig.cap = "Valley delineation without (top) and with (bottom) waterbodies and channel buffer. Green = valley, blue = streams."}
...
if (nrow(waterbodies) > 0) {
  plot(st_geometry(waterbodies), add = TRUE, border = "cyan", lwd = 0.8)
}
```
